...
for random_seed in args.random_state_list:
    ...
    folder_path = f'NEW_CORR_KNN_kmeans_class_results/{dataset_name}'
    ...
    for perc, sample_size in zip(reversed(percentages), reversed(sample_sizes)):
        ...
        # Initialize overall times
        total_kmeans_fit_time = {'wall': 0, 'process': 0, 'perf': 0}
        total_selection_time = {'wall': 0, 'process': 0, 'perf': 0}

        for label in np.unique(y_train):
            if perc == 1.0:
                continue
            X_class = X_train[y_train == label]  # Subset data for current class
            n_clusters = max(1, int(np.ceil(sample_size / len(np.unique(y_train)))))  # k = ⌈˜n / c⌉
            #print(n_clusters)
            
            if n_clusters > len(X_class):
                n_clusters = len(X_class)
            
            # Measure time for KMeans fitting
            def fit_kmeans():
                kmeans = KMeans(n_clusters=n_clusters, random_state=random_seed)
                return kmeans.fit(X_class)

            _, kmeans_fit_time_label = measure_time(fit_kmeans)  
            kmeans_fit_time[label] = kmeans_fit_time_label  # Save time for each class
            
            # Update overall KMeans fitting time
            total_kmeans_fit_time['wall'] += kmeans_fit_time_label['wall']
            total_kmeans_fit_time['process'] += kmeans_fit_time_label['process']
            total_kmeans_fit_time['perf'] += kmeans_fit_time_label['perf']
            kmeans = KMeans(n_clusters=n_clusters, random_state=random_seed)
            kmeans.fit(X_class)  # Fit KMeans again for selecting the closest points

            cluster_labels = kmeans.predict(X_class)

            def find_closest_points():
                selected_indices_local = []
                for cluster_idx in range(n_clusters):
                    cluster_members = np.where(cluster_labels == cluster_idx)[0]
                    if len(cluster_members) == 0:
                        continue  # Skip empty clusters
                    
                    # Compute distance to centroid
                    distances = np.linalg.norm(X_class[cluster_members] - kmeans.cluster_centers_[cluster_idx], axis=1)
                    closest_idx = cluster_members[np.argmin(distances)]
                    selected_indices_local.append(np.where(y_train == label)[0][closest_idx])  # Map back to original indices
                return selected_indices_local
                
            selected_indices_local, selection_time_label = measure_time(find_closest_points)
            selected_indices.extend(selected_indices_local)  # Add the selected indices from this label
            selection_time[label] = selection_time_label  # Save the time for finding closest points
            
            # Update overall selection time
            total_selection_time['wall'] += selection_time_label['wall']
            total_selection_time['process'] += selection_time_label['process']
            total_selection_time['perf'] += selection_time_label['perf']
        
        selected_indices = np.array(selected_indices)
        
        # Add KMeans fitting time and selection time to results
        results.update({f'kmeans_fit_time_{label}': time for label, time in kmeans_fit_time.items()})
        results.update({f'selection_time_{label}': time for label, time in selection_time.items()})

        selection_time_k = {k: total_kmeans_fit_time[k] + total_selection_time[k] for k in total_kmeans_fit_time}

        # Update results with selection_time_k
        results.update({f'selection_time_{k}': v for k, v in selection_time_k.items()})
                
        # Select the subset based on the selected indices
        if perc == 1.0:
            X_train_N, y_train_N = X_train, y_train
        else:
            X_train_N, y_train_N = X_train[selected_indices], y_train[selected_indices]
    ...
    results_df = df
    folder_path = f'NEW_CORR_KNN_kmeans_class_results/{dataset_name}'  # Corrected variable name
